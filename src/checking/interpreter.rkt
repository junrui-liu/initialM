  ...
  (for*/permuted ([command (ag:visitor-commands visitor)])
    (match command
      ...
      [(ag:eval attr)
       ...
       (define tmp0 (tree-select/ready self attr))
       (define tmp000 (tree-select/ready self attr))
       (define tmp1 (evaluate self (ag:rule-formula rule)))
       (printf ">>> tmp000 is: ~a\n" tmp000)
       (printf ">>> rule is:~a, tmp0 is:~a, which will be set as:~a\n" rule tmp0 tmp1)
       (set-box! tmp0 tmp1)
       ; (set-box! (tree-select/ready self attr)
...
(define (iterate self child order commands trav)
                     ...
                     (match command
                       ...
                       [(ag:eval attr)
                        ...
                        (if (ag:rule-folds? rule)
                          ...
                          (begin
                            (printf "LOC:3\n")
                            (set-box! (tree-select/ready self attr #:iterator child #:cursor node)
  ...
  (for ([(attr value) (in-dict state#)])
    (printf "LOC:1\n")
    (set-box! (tree-select/ready self attr) (unbox value))))
...
(define (evaluate self term #:iterator [iter #f] #:cursor [cur #f] #:accumulator [acc #f])
  (define/match (recur term)
    [((ag:const val))
     val]
    [((ag:field attr))
     (unbox (tree-select/ready self attr #:iterator iter #:cursor cur))]
    [((ag:accum attr))
    ...
    [((ag:index/first (cons child field) default))
     ...
     (if (null? nodes)
         (recur default)
         (unbox (tree-ref/ready (first nodes) field)))]
    [((ag:index/last (cons child field) default))
     (define nodes (tree-ref/child self child))
     (if (null? nodes)
         (recur default)
         (unbox (tree-ref/ready (last nodes) field)))]
    [((ag:ite if then else))
