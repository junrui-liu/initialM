...
void _fuse__F1F2(Vertex *_r, double _f1_parentPotential,
...
void _fuse__F1F2(Vertex *_r, double _f1_parentPotential,
  ...
  if (truncate_flags & 0b1) {
    ...
    _r->collectWellSeperatedNodes();
    if (true) {
      _r->readInteractionList();
      _r->updatePotentialUsingInteractionList();
    }
  }
_label_B1F0_Exit:
  double _f1_tmp;
  if (truncate_flags & 0b10) {
    ...
    _r->incrPotential_step3(_f1_parentPotential);
    _f1_tmp = _r->readPotential();
  }
_label_B1F1_Exit:
  if ((truncate_flags & 0b11)) /*call*/ {
    ...
    _fuse__F1F2(_r->c4, _f1_tmp, AdjustedTruncateFlags);
  }
  if ((truncate_flags & 0b11)) /*call*/ {
    unsigned int AdjustedTruncateFlags = 0;
    AdjustedTruncateFlags <<= 1;
    ...
    AdjustedTruncateFlags <<= 1;
    AdjustedTruncateFlags |= (0b01 & (truncate_flags >> 0));
    _fuse__F1F2(_r->c3, _f1_tmp, AdjustedTruncateFlags);
  }
  if ((truncate_flags & 0b11)) /*call*/ {
    unsigned int AdjustedTruncateFlags = 0;
    AdjustedTruncateFlags <<= 1;
    ...
    AdjustedTruncateFlags <<= 1;
    AdjustedTruncateFlags |= (0b01 & (truncate_flags >> 0));
    _fuse__F1F2(_r->c1, _f1_tmp, AdjustedTruncateFlags);
  }
  if ((truncate_flags & 0b11)) /*call*/ {
    unsigned int AdjustedTruncateFlags = 0;
    AdjustedTruncateFlags <<= 1;
    ...
    AdjustedTruncateFlags <<= 1;
    AdjustedTruncateFlags |= (0b01 & (truncate_flags >> 0));
    _fuse__F1F2(_r->c2, _f1_tmp, AdjustedTruncateFlags);
  }
  return;
};
